...
API credentials (`CLAUDE_CODE_OAUTH_TOKEN` or `ANTHROPIC_API_KEY`) are read from the host's `.env` / environment and passed directly as env vars to agent containers at spawn time.
